...
class OmniglotNShotDataset():
    def __init__(self,
        ...
        self.x_train, self.x_test, self.x_val = self.x[:1200], self.x[
            1200:1500], self.x[1500:]
        self.normalization()
    ...
    def normalization(self):
        ...
        print("train_shape", self.x_train.shape, "test_shape",
              self.x_test.shape, "val_shape", self.x_val.shape)
    ...
    def load_data_cache(self, data_pack):
        ...
        for sample in range(200):
            ...
            target_x = np.zeros(
                (self.batch_size, self.samples_per_class, 28, 28, 1),
                dtype=np.int)
            target_y = np.zeros(
                (self.batch_size, self.samples_per_class), dtype=np.int)
            for i in range(self.batch_size):
                pinds = np.random.permutation(n_samples)
                classes = np.random.choice(data_pack.shape[0],
                                           self.classes_per_set, False)
                # select 1-shot or 5-shot classes for test with repetition
                x_hat_class = np.random.choice(classes, self.samples_per_class,
                                               True)
                ...
                for j, cur_class in enumerate(classes):  # each class
                    if cur_class in x_hat_class:
                        ...
                        example_inds = np.random.choice(
                            data_pack.shape[1], self.samples_per_class, False)

                    # meta-training
                    for eind in example_inds[:self.samples_per_class]:
                        support_set_x[i, pinds[ind], :, :, :] = data_pack[
                            cur_class][eind]
                        support_set_y[i, pinds[ind]] = j
                        ind = ind + 1
                    # meta-test
                    for eind in example_inds[self.samples_per_class:]:
                        target_x[i, pinds_test[ind_test], :, :, :] = data_pack[
                            cur_class][eind]
    ...
    def __get_batch(self, dataset_name):
        ...
        :return:
        """
        if self.indexes[dataset_name] >= len(
                self.datasets_cache[dataset_name]):
            self.indexes[dataset_name] = 0
            self.datasets_cache[dataset_name] = self.load_data_cache(
                self.datasets[dataset_name])
        next_batch = self.datasets_cache[dataset_name][self.indexes[
            dataset_name]]
